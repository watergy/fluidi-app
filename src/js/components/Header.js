...
class Header extends Component {
  ...
  componentDidMount() {
    ...
    State.local.get("activeRoute").on(
      this.sub((activeRoute) => {
        ...
        if (
          ...
          if (
            activeRoute.indexOf("/chat/") === 0 &&
            Session.getKey() &&
            this.chatId === Session.getKey().pub
          ) {
            const title = html`<b style="margin-right:5px">📝</b>
              <b>${t("note_to_self")}</b>`;
            this.setState({ title });
          } else {
            State.local
  ...
  render() {
    ...
    const searchBox = this.chatId
      ? ""
      : html` <${SearchBox} focus=${!!this.state.showMobileSearch} /> `;
      ...
      <div class="header-content">
        ...
        >
          ${this.state.title && chatting
            ? html` <div class="name">${this.state.title}</div> `
            : ""}
          ${isTyping
            ? html`<small class="typing-indicator">${t("typing")}</small>`
            : ""}
          ${this.state.about
            ? html`<small class="participants">${this.state.about}</small>`
            : ""}
          ${this.chatId
            ? html`<small class="last-seen">${onlineStatus || ""}</small>`
            : ""}
          ${chatting
            ? ""
            : html`
                ...
                <div
                  class="mobile-search-btn ${this.state.showMobileSearch
                    ? "hidden"
        ...
        ${chat && this.chatId !== key && !chat.uuid
          ? html`
              <a
                ...
                id="start-video-call"
                onClick=${() =>
                  State.local.get("outgoingCall").put(this.chatId)}
              >
              ...
              <a
                ...
                onClick=${() =>
                  State.local
                    ...
                    .put(!this.state.showParticipants)}
              >
        ...
        <a
          href="/profile/${key}"
          onClick=${() => State.local.get("scrollUp").put(true)}
          class="hidden-xs ${activeRoute && activeRoute === `/profile/${key}`
            ? "active"
            : ""} my-profile"
        >
