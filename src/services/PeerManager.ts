...
async function addPeer(peer) {
  ...
  if (peer.visibility === "public") {
    ...
    let encryptedUrlHash = await Gun.SEA.work(encryptedUrl, null, null, {
      name: "SHA-256",
    });
    State.public
      .user()
      .get("peers")
      .get(encryptedUrlHash)
      .put({ url: peer.url, lastSeen: new Date().toISOString() });
  }
...
function getRandomPeers() {
  const connectToLocalElectron =
    Helpers.isElectron &&
    knownPeers[ELECTRON_GUN_URL] &&
    knownPeers[ELECTRON_GUN_URL].enabled !== false;
  const sampleSize = connectToLocalElectron
    ? Math.max(maxConnectedPeers - 1, 1)
    : maxConnectedPeers;
  const sample = _.sampleSize(
    Object.keys(
      _.pickBy(knownPeers, (p, url) => {
        const mixedContent =
          window.location.protocol === "https:" && url.indexOf("http:") === 0;
        return (
          !mixedContent &&
          p.enabled &&
          !(Helpers.isElectron && url === ELECTRON_GUN_URL)
        );
      })
    ),
    sampleSize
...
let askForPeers = _.once((pub) => {
  if (!Session.settings.local.enablePublicPeerDiscovery) {
    return;
  }
  _.defer(() => {
    State.public
...
function checkGunPeerCount() {
  ...
  if (connectedPeers.length < maxConnectedPeers) {
    let unconnectedPeers = _.filter(Object.keys(knownPeers), (url) => {
      let addedToGun =
        _.map(Object.values(peersFromGun), "url").indexOf(url) > -1;
